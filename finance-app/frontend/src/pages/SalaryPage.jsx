...
export default function SalaryPage() {
  ...
  if (loading) return <p className="text-gray-500 text-sm p-6">Loading salary data…</p>;
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-lg font-semibold text-gray-200">Salary History</h2>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <SKpi label="Current Company" value={latest?.company ?? "—"}
          sub={`${JURISDICTION_LABELS[latest?.jurisdiction] ?? ""} ${latest?.jurisdiction}`} color="text-brand-400" />
        <SKpi label="Total Net Earned" value={`${fmt(Math.round(totalNet))}`}
          sub="since Aug 2019" color="text-gray-200" />
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
        <h3 className="text-sm font-semibold text-gray-300 mb-1">Gross Pay Breakdown per Month</h3>
        <p className="text-xs text-gray-600 mb-4">Stacked by component · dashed line = net after tax</p>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-800">
          <h3 className="text-sm font-semibold text-gray-300">Full History</h3>
        </div>
              ...
              {[...salary].reverse().map((row, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                    <td className="px-3 py-2 text-gray-400 font-mono">{row.date.slice(0, 7)}</td>
                    ...
                    <td className="px-3 py-2 text-gray-300 font-mono text-right">{fmt(row.gross)}</td>
                    ...
                    <td className="px-3 py-2 text-gray-600 max-w-48 truncate">{row.comment ?? ""}</td>
                  </tr>
                );
              })}
            </tbody>
...
function SalaryTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-lg p-3 text-xs space-y-1">
      ...
      <div className="border-t border-gray-800 pt-1 mt-1 space-y-0.5">
        ...
        <p className="text-gray-300 font-semibold border-t border-gray-800 pt-1">Total gross: {fmt(total)}</p>
        <p className="text-gray-500">Net: ~{fmt(d?.net)}</p>
      </div>
    </div>
  );
}
