    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-sm px-4 py-1.5 rounded-lg bg-brand-600 hover:bg-brand-700 transition-colors"
        >
      ...
      {showForm && (
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 space-y-4">
          <h3 className="font-semibold text-gray-200">New budget envelope</h3>
                ...
                <button
                  ...
                  className={`text-xs px-2.5 py-1 rounded-full border transition-colors ${
                    form.category_filter.includes(c)
                      ? "border-brand-500 bg-brand-600/30 text-brand-400"
          ...
          <button
            ...
            className="text-sm px-4 py-2 rounded-lg bg-brand-600 hover:bg-brand-700 disabled:opacity-40 transition-colors"
          >
            Create envelope
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        {budgets.map((env) => {
          ...
          return (
            <div key={env.id} className="bg-gray-900 rounded-xl border border-gray-800 p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="font-semibold text-gray-200">{env.name}</p>
      ...
      {budgets.length === 0 && !showForm && (
        <p className="text-gray-500 text-sm">No envelopes yet. Create one for a trip or monthly goal.</p>
      )}
    </div>
  );
}

const input = "w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-1.5 text-sm focus:border-brand-500 focus:outline-none";
